...
--========================================================================
--========================================================================
-- Nuevas columnas Maestro Cuenta: Process Date, Estado Cuenta, Fecha Vinculación (date_account_opened)

ALTER TABLE public.tbvw_maestro_cuentas_pershing ADD COLUMN estado_cuenta VARCHAR(100) DEFAULT NULL;
SELECT DISTINCT estado_cuenta FROM public.tbvw_maestro_cuentas_pershing;

--Función que devuelve estado de la cuenta desde tablas materializadas
CREATE or replace function public.fn_estado_cuenta_from_tbvw(
    _account_no VARCHAR(100),
    _opt_process_date VARCHAR(100)
) RETURNS VARCHAR(100)
    LANGUAGE plpgsql
as
$$
...
DROP function IF EXISTS public.fn_reporte_maestro_datos_clientes;
DROP function IF EXISTS public.fn_reporte_maestro_datos_relacionados;
DROP VIEW IF EXISTS public.vw_reporte_maestro_datos_relacionados;
DROP VIEW IF EXISTS public.vw_reporte_maestro_datos_clientes;
...
FROM (SELECT 'B'::text                                                                                                  AS src_vw,
             ...
             vw_acct.process_date                                                  AS process_date,
...
--TODO: Re crear con Diff Schemas

--Actualizamos data histórica de estado_cuenta
SELECT * INTO zz_backup.tbvw_maestro_cuentas_pershing_20250901 FROM public.tbvw_maestro_cuentas_pershing;

SELECT process_date, estado_cuenta, count(*) FROM public.tbvw_maestro_cuentas_pershing group by process_date, estado_cuenta order by process_date, estado_cuenta;

UPDATE public.tbvw_maestro_cuentas_pershing
    SET estado_cuenta = public.fn_estado_cuenta_from_tbvw(account_no, process_date)
;

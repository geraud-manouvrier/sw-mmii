...
@Component
public class IngresosEgresosMensualExcel {
    ...
    public void generaReportesExcel(Date processDate) throws QandeMmiiException {
        appConfig.customLog.info("Iniciando generación de Reporte Mensual I/E para fecha :"+processDate);
        this.generaReporteMensual(CUSTODIO_STONEX, processDate);
        appConfig.customLog.info("Generación Reporte Mensual I/E finalizada");
    }

    public void generaReporteMensual(String custodio, Date dateTo) throws QandeMmiiException {
        this.guardaExcel(this.reporteMensual(custodio, dateTo), reportesIngresosEgresosHelper.generaNombreReporteMensual(dateTo));
    }
    ...
    private Workbook reporteMensual(String custodio, Date dateTo) {
        appConfig.customLog.info("Iniciando generación de Reporte Mensual ["+custodio+"] para fecha :["+dateTo+"]");
        ...
        var reporteMensual = reporteIngresosEgresosService.reporteMensual(custodio, dateTo);
        ...
        for (VwReporteIngresosEgresosMensual fila : reporteMensual) {
            appConfig.customLog.info("Procesando fila ID ["+fila.getRowId()+"]");


            if ( ! mesActual.equals(fila.getMes()) ) {
                dataRow = sheet.createRow(row++);
                anioActual  = fila.getAnio();
                mesActual   = fila.getMes();
                numOficina=1;
                var totales = reporteIngresosEgresosService.totalReporteMensualByCustodioAndPeriodo(custodio, anioActual, mesActual);

                reportesIngresosEgresosHelper.addCell(dataRow, 0, calendarioHelper.nombreMes(fila.getMes(), "ES"), null);
                reportesIngresosEgresosHelper.addCell(dataRow, 1, fila.getAnio(), null);
